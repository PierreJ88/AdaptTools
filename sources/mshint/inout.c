...
int loadSol(pSol sol,char *filename) {
  ...
  /* read mesh solutions */
  if ( sol->np ) {
    sol->valp1 = (double*)M_calloc(sol->np+1,sol->size[0]*sizeof(double),"inout");
    assert(sol->valp1);
  ...
  if ( sol->ne ) {
    sol->valp0 = (double*)M_calloc(sol->ne+1,sol->size[1]*sizeof(double),"inout");
    assert(sol->valp0);
  ...
  GmfGetLin(inm,GmfIterations,ibuf);
  sol->iter = ibuf[0];
  ...
  if ( sol->ver == GmfFloat ) {
    ...
    GmfGetLin(inm,GmfTime,dbuf);
    sol->time = dbuf[0];
  }
  sol->iter = GmfStatKwd(inm,GmfIterations);
  sol->time = GmfStatKwd(inm,GmfTime);
